...
import config
import utils
#from thirdEncoder import TreeLSTM
...
class GAT(nn.Module): # as a contrast
    ...
    def forward(self, x, adj):
        x = self.dropout1(x)  # [32,21,512]  [32,21,21]
        #print(x.size())
...
class treeEncoder(nn.Module):
    def __init__(self, vocab_size):
        ...
        self.embedding = nn.Embedding(vocab_size, config.embedding_dim)
        self.gat = GAT(config.nfeat, config.nhid, config.hidden_size, config.dropout, config.alpha,
                       config.nheads)
    ...
    def forward(self, inputs: torch.Tensor, seq_lens: torch.Tensor, adjacent: torch.Tensor):
        ...
        embedded = torch.zeros(config.batch_size, config.max_tree_length + 1, config.embedding_dim)
        _, size_em, _ = embedded0.size()
